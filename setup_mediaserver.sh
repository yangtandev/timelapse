...
# Patch driver
cd nvidia-patch && sudo ./patch.sh && cd $HOME/timelapse || exit

# Clone ffnvcodec
git clone https://git.videolan.org/git/ffmpeg/nv-codec-headers.git

# Install ffnvcodec
cd nv-codec-headers && sudo make install && cd $HOME/timelapse || exit
...
sudo apt-get -y update -qq && sudo apt-get -y install autoconf automake build-essential cmake git-core libass-dev libfreetype6-dev libgnutls28-dev libmp3lame-dev libsdl2-dev libtool libva-dev libvdpau-dev libvorbis-dev libxcb1-dev libxcb-shm0-dev libxcb-xfixes0-dev meson ninja-build pkg-config texinfo wget yasm zlib1g-dev

# Enter ZLMediaKit directory
cd $HOME/timelapse/ZLMediaKit || exit
